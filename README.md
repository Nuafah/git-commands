...
1. When using Git locally, what are these?  Define each one in a sentence
   * Staging area - ``` The area that stored the changed files wating to be commit. ```
   * Working copy - ``` The copy of files you work on. ```
   * master - ``` Name of initial branch. ```
   * HEAD - ``` A label to indicate which branch you are working on. ```
...
3. There are 2 ways to create a local Git repository.  Briefly descibe each one:
   - Use ```git init``` to create .git directory in current directory.
   - Clone a remote repo to your current directory by using ```git clone url localDirectoryName```.
...
3. Merge other branch to current branch:
   ```
...
1. Display the current state of your working directory and staging area:
   ```
   git status
   ```
2. Display the commit history in a Git repository:
   ```
   git log
   ```
...
* Learn Git Visually:
  * [Git basics](https://www.atlassian.com/git) visual tutorial for basic git commands
